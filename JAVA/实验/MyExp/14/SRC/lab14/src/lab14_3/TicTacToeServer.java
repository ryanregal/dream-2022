...
{
   private String[] board = new String[ 9 ]; // 井字棋盘
   private JTextArea outputArea; //用于输出动作
   private Player[] players; // 玩家数组
   private ServerSocket server; // 服务器套接字与客户端连接
   private int currentPlayer; // 跟踪当前移动的玩家
   private final static int PLAYER_X = 0; //第一个玩家的常数
   private final static int PLAYER_O = 1; // 第二个玩家的常数
   private final static String[] MARKS = { "X", "O" }; // 标记数组
   private ExecutorService runGame; //下一步玩家
   private Lock gameLock; // 锁定游戏以进行同步
   private Condition otherPlayerConnected; // 等待其他玩家
   private Condition otherPlayerTurn; // 等待轮到其他玩家
   
   // 设置井字游戏服务器和显示消息的 GUI
   public TicTacToeServer()
   {
      super( "Tic-Tac-Toe Server" ); //窗口标题，继承自JFrame

      //为每个玩家添加一个线程池
      runGame = Executors.newFixedThreadPool( 2 );
      gameLock = new ReentrantLock(); // 为游戏创建锁
   ...
   public void execute()
   {
      ...
      gameLock.lock(); // 锁定游戏以通知玩家 X 的线程

      try
      {
         players[ PLAYER_X ].setSuspended( false ); // 恢复玩家 X
         otherPlayerConnected.signal(); // 唤醒玩家 X 的线程
      } // end try
      finally
      {
         gameLock.unlock(); //向玩家 X 发出信号后解锁游戏
      }
   }
