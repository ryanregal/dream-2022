...
public class notepadController extends Stage implements Initializable
{
	...
	public void initialize(URL location, ResourceBundle resources) 
	{
		...
		textArea.setOnMouseClicked(new EventHandler<MouseEvent>()
		{
			...
			{
				statusLabelUpdate();//更新状态栏
				myEdit();//设置编辑菜单里部分选项是否可选
			}
		});
	...
	private void newFile()
	{
        ...
        alert.setTitle("冰熊记事本");
        alert.setHeaderText("你想更改保存吗？");
	...
	private void open()
	{
		...
		else
		{
			...
			try
			{
				...
				//它使用的字符集可以由名称指定，也可以显式给出，或者可以接受平台的默认字符集。
				InputStreamReader streamReader = new InputStreamReader(new FileInputStream(file));
				//BufferedReader:创建一个使用默认大小的输入缓冲区的缓冲字符输入流
	...
	public void save()
	{
		...
		if(currentPath == null) saveAs();
			...
			try{
				...
				fw.write(textArea.getText());//写入textArea的文字
				fw.flush();//刷新
				stage.setTitle(currentPath);//title增加为当前路径
			}
			catch(Exception e){}
			finally
			{
	...
	private void saveAs()
	{
        ...
        if (file == null) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("警告");
            alert.setHeaderText("当前文件未保存 Σ( ° △ °|||)︴");
            alert.setContentText("");
            alert.showAndWait();
        }
        else{
	...
	public void replace()
	{
		...
		strB = global.search;//查找
		strC = global.replace;//替换
		...
		if(a > -1)
		{
			...
			textArea.positionCaret(a);
			b = strB.length();
			textArea.selectRange(a, a + b);//选择区域
			//尝试替换
			try
			{
				//没有替换文本，则清除内容
	...
	private void about()
	{
		Alert alert = new Alert(Alert.AlertType.INFORMATION); // 创建一个消息对话框
        alert.setTitle("冰熊记事本");
        alert.setHeaderText("这是一个模仿Windows notepad的Java记事本程序");
